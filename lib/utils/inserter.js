...
function createManifest(directory) {
    filePath = directory.concat("/manifest")
...
function getManifestContent() {
    versionNumber = splitVerison()

    return [
        createManifestEntry('title', properties.name),
        createManifestEntry('major_version', versionNumber[0]),
        createManifestEntry('minor_version', versionNumber[1]),
        createManifestEntry('build_version', versionNumber[2]),
        createManifestEntry('mm_icon_focus_hd', ''),
        createManifestEntry('mm_icon_focus_sd', ''),
        createManifestEntry('splash_screen_hd', ''),
        createManifestEntry('splash_screen_sd', '')
    ].join('\n')
}

function createManifestEntry(key, value) {
    return [
        key,
        value
    ].join('=')
...
function splitVerison() {
    return properties.version.split('.')
}
